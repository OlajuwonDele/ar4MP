...
.. code-block:: bash
    ...
    # Local // TESTED AND WORKING
        python scripts/ray/tuner.py \
        --cfg_file scripts/ray/hyperparameter_tuning/vision_ar4_cfg.py \
        --cfg_class AR4MPJobCfg \
        --run_mode local \
        --workflow scripts/rl_games/train.py \
    ...
    # Local with a custom progress reporter
        ...
        --workflow scripts/rl_games/train.py \
        --num_workers_per_node 1
        --progress_reporter CustomAR4MPProgressReporter
    # Remote (run grok cluster or create config file mentioned in :file:`submit_job.py`) // NOT TESTED YET
...
"""
DOCKER_PREFIX = "/workspace/ar4mp/"
BASE_DIR = os.getcwd()
PYTHON_EXEC = sys.executable  # use the Python that runs this script
WORKFLOW = os.path.join(BASE_DIR, "scripts/rl_games/train.py")
...
if __name__ == "__main__":
    ...
    if args.run_mode == "remote":
        ...
        PYTHON_EXEC = DOCKER_PREFIX + PYTHON_EXEC
